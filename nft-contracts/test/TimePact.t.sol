...
contract DealClientTest is DSTest, ERC721Holder {
    ...
    function createDealRequest() public view returns (IDeal.DealRequest memory) {
        ...
        IDeal.DealRequest memory idRequest = IDeal.DealRequest({
            piece_cid: testCID,
            piece_size: 2048,
            verified_deal: false,
            label: "",
            ...
            client_collateral: 0,
            extra_params_version: 0,
            extra_params: IDeal.ExtraParamsV1({
                location_ref: "",
                car_size: 0,
                skip_ipni_announce: false,
                remove_unsealed_copy: false
    ...
    function testDealNoClient() public {
        ...
        (string memory creator, uint64 time, , , ) = timePact.tokenInfo(0);
        emit log_uint(time);
        emit log_string(creator);
    }
