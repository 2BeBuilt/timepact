...
contract DealClientTest is DSTest, ERC721Holder {
    ...
    function testMakeDealProposalWithTimePact() public {
        require(timePact.dealsLength() == 0, "Expect no deals");
        timePact.pact("pcideeee", "creatoor", 0, createDealRequest());
        require(timePact.dealsLength() == 1, "Expect one deal");

        RequestId memory proposalIdSet = timePact.getProposalIdSet(testCID);
        require(proposalIdSet.valid, "expected to have valid Proposal");
        DealRequest memory deal = timePact.getDealByIndex(0);
        require(deal.piece_size == 2048, "unexpected cid size in client after setting");

        ProviderSet memory providerSet = timePact.getProviderSet(testCID);
        require(!providerSet.valid, "should not be valid before a cid is authorized");

        // non-added cid has expected state
        RequestId memory proposalIdSetShort = timePact.getProposalIdSet(testShortCID);
        require(!proposalIdSetShort.valid, "expected to have valid Proposal");
        ProviderSet memory providerSetShort = timePact.getProviderSet(testShortCID);
